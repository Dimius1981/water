<div class="row">
  <div class="col-md-1">
  </div>
  <div class="col-md-10">
    <br>
    <div class="row">
      <div class="col-md-3">
        <h2>Пользователи <button class="modaladdrec" data-bs-toggle="modal" data-bs-target="#addSensorsModal" ><img src="./templates/images/adduser.png"></button></h2>
      </div>
      <div class="col-md-9">
        <ul class="nav nav-pills">
          <li class="nav-item">
            <a class="nav-link" href="/">Home</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="?page=downloads">Загрузка</a>
          </li>
          <li class="nav-item">
            <a class="nav-link" href="?page=logs">Журнал</a>
          </li>
          <li class="nav-item">
            <a class="nav-link active" href="?page=users">Пользователи</a>
          </li>
        </ul>
      </div>
      <hr>
      <div class="tab-content">

        <table class="table padd table-borderless">
          <thead class="table ">
            <tr>
              <th>Статус</th>
              <th>Имя</th>
              <th>Логин</th>
              <th>Последний сеанс</th>
              <th>email</th>
              <th>Доступ</th>
              <th></th>
            </tr>
          </thead>
          <tbody>
            <tr>
              <td>1</td>
              <td>Admin</td>
              <td>Admin</td>
              <td>11.11.2022 19:53:46</td>
              <td>admin@admin</td>
              <td>
                <div class="form-check form-switch">
                  <input class="form-check-input" type="checkbox" id="mySwitch" name="darkmode" value="yes" checked>
                </div>
              </td>
              <td>
                <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#usereditModal"><img src="templates/image/pen.png"></button>
                <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deluserModal"><img src="templates/image/del.png"></button>
              </td>
            </tr>
            <tr>
              <td>1</td>
              <td>Admin</td>
              <td>Admin</td>
              <td>11.11.2022 19:53:46</td>
              <td>admin@admin</td>
              <td>
                <div class="form-check form-switch">
                  <input class="form-check-input" type="checkbox" id="mySwitch" name="darkmode" value="yes" checked>
                </div>
              </td>
              <td>
                <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#usereditModal"><img src="templates/image/pen.png"></button>
                <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deluserModal"><img src="templates/image/del.png"></button>
              </td>
            </tr>
            <tr>
              <td>1</td>
              <td>Admin</td>
              <td>Admin</td>
              <td>11.11.2022 19:53:46</td>
              <td>admin@admin</td>
              <td>
                <div class="form-check form-switch">
                  <input class="form-check-input" type="checkbox" id="mySwitch" name="darkmode" value="yes" checked>
                </div>
              </td>
              <td>
                <button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#usereditModal"><img src="templates/image/pen.png"></button>
                <button type="button" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#deluserModal"><img src="templates/image/del.png"></button>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>
  </div>
  <div class="col-md-1">
  </div>
</div>